...
import { useGetIssueById } from '../hooks/useGetIssues';
import { formatFullDate, timeAgo } from '../utils/dateUtils';

const IssueDetails: React.FC = () => {
  const { id } = useParams(); 
  const {issue, loading} = useGetIssueById(parseInt(id||''));
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 relative">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-6 border-b border-gray-100 pb-6">
                <div className="flex items-center">
                  ...
                  <span>by <span className="font-semibold text-gray-700">{issue?.user.name}</span></span>
                </div>
                <div className="flex items-center">
                  <Calendar size={16} className="mr-1.5 text-gray-400" />
                  <span>{formatFullDate(issue?.created_at || '')}</span>
                </div>
              </div>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              ...
              <div className="space-y-4">
                ...
                <DetailRow label="Created" value={timeAgo(issue?.created_at || '')} />
                <DetailRow label="Comments" value="0" />
                {/* <DetailRow label="Media" value={`${issue?.image.length} files`} /> */}
